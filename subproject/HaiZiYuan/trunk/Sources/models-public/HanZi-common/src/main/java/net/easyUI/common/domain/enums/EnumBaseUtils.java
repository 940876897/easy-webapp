package net.easyUI.common.domain.enums;
...
public class EnumBaseUtils {
	...
	/**
	 * 将EnumBase.getCode()作为Key,EnumBase.getDesc()作为value,存放在Map中并返回，其中，Key转为String类型
	 * 
	 * @param <T>
	 * @param values
	 * @return
	 */
	public static <T extends EnumBase> LinkedHashMap toStrMap(T[] values) {
		LinkedHashMap map = new LinkedHashMap();
		for (EnumBase item : values) {
			map.put(item.getCode()+"", item.getDesc());
		}
	...
	/**
	 * 将EnumBase.getCode()作为Key,EnumBase对象作为value,存放在Map中并返回
	 * 
	 * @param values
	 * @return
	 */
	public static <T extends EnumBase> LinkedHashMap toObjMap(T[] values) {
		LinkedHashMap map = new LinkedHashMap();
		for (EnumBase item : values) {
