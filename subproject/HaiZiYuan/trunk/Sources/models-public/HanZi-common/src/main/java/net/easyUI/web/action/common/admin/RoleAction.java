...
public class RoleAction extends BaseAction {
    ...
    public String roleIndexAjax(@ModelAttribute("query") RoleQuery query, UserAgent user,
                                ModelMap model) {
        ServiceResult<DwzPage<Role>> result = roleService.pageQuery(new ServiceRequest(query));
    ...
    public String roleAddAjax(UserAgent user, ModelMap model) {
        Role role = new Role();
        model.put("role", role);
    ...
    public DwzJson roleMetaDel(@PathVariable("id") Long id,
                               ...
                               UserAgent user, ModelMap model) {
        DwzJson dwzJson;
        if (id == null || id < 0) {
            dwzJson = new DwzJson("300", this.getMessageSource().getMessage(
                "delete.error.parameter.ids.null", null, this.getThisLocale()));
            return dwzJson;
        }
        WebConfigQuery query = new WebConfigQuery();
        query.setId(id);
        ServiceResult<Integer> result = webConfigService.remove(new ServiceRequest(query, user));
        if (result.getErrorNO() != null) {
            dwzJson = new DwzJson("300", this.messageSource.getMessage(result.getErrorInfo(),
                result.getMsgArgs(), this.getThisLocale()));
        } else {
            if (result.getDataObj() > 0) {
                dwzJson = new DwzJson("200", this.messageSource.getMessage("operation.success",
                    result.getMsgArgs(), this.getThisLocale()), dwzId);
            } else {
                dwzJson = new DwzJson("300", this.getMessageSource().getMessage("delete.error",
                    result.getMsgArgs(), this.getThisLocale()));
            }
        }
    ...
    public String rolePermissionAddAjax(@PathVariable("id") Long id,
        ...
        if (role == null) {
            dwzJson = new DwzJson("300", this.messageSource.getMessage("ServiceError.not_find",
    ...
    DwzJson rolePermissionAddJson(@PathVariable("roleId") Long roleId,
                                  ...
                                  UserAgent user, ModelMap model) {
        DwzJson dwzJson;
        Role role = roleService.queryOne(new ServiceRequest(roleId)).getDataObj();
        if (role == null) {
            dwzJson = new DwzJson("300", this.messageSource.getMessage("ServiceError.not_find",
                null, this.getThisLocale()));
            return dwzJson;
        ...
        if (CollectionUtils.isEmpty(permissionKey_in))
            return new DwzJson();
        String permissionKeysOld = role.getMetaMap().get(ConstantDTO.roleMeta_Key_PERMISSIONKEY);
        String permissionKeys = null;
        for (String permissionKey : permissionKey_in) {
            if (StringUtils.isBlank(permissionKeys))
                permissionKeys = permissionKey;
            else
                permissionKeys = permissionKeys.trim() + "," + permissionKey.trim();
        }
        if (StringUtils.isBlank(permissionKeys))
            return new DwzJson();
        if (StringUtils.isBlank(permissionKeysOld)) {
            ...
            webConfig.setCfgName(role.getRoleKey() + "_" + ConstantDTO.roleMeta_Key_PERMISSIONKEY);
        ...
        dwzJson = new DwzJson("200", this.messageSource.getMessage("operation.success", null,
            this.getThisLocale()), dwzId, EnumDwzJsonCallbackType.CLOSECURRENT.getCode());
        return dwzJson;
    }
    ...
    public DwzJson rolePermissionDelJson(@PathVariable("roleId") Long roleId,
        ...
        if (StringUtils.isEmpty(permissionKey))
            return new DwzJson();
        Role role = roleService.queryOne(new ServiceRequest(roleId, userAgent)).getDataObj();
        if (role == null) {
            dwzJson = new DwzJson("300", this.messageSource.getMessage("ServiceError.not_find",
                null, this.getThisLocale()));
            return dwzJson;
        ...
        for (Permission permission : role.getPermissions()) {
            ...
            if (StringUtils.isBlank(permissionKeys))
                ...
                permissionKeys = permissionKeys.trim() + "," + permission.getPermissionKey().trim();
        }

        // 修改.
        WebConfigQuery webConfigQuery = new WebConfigQuery();
        webConfigQuery.setCfgGroup(EnumWebCfgGroup.ROLEMETA.getCode());
        webConfigQuery.setCfgName(role.getRoleKey() + "_" + ConstantDTO.roleMeta_Key_PERMISSIONKEY);
        WebConfig webConfig = new WebConfig();
        webConfig.setCacheType(EnumWebCfgCacheType.NONE.getCode());
        webConfig.setCfgGroup(EnumWebCfgGroup.ROLEMETA.getCode());
        webConfig.setCfgValue(permissionKeys);
    ...
    @ResponseBody
    public DwzJson roleSave(@ModelAttribute("role") Role role, UserAgent user, ModelMap model) {
        DwzJson dwzJson;
        if (role == null) {
            dwzJson = new DwzJson("300", this.messageSource.getMessage("operation.failed", null,
                this.getThisLocale()));
            return dwzJson;
        }
        if (role.getId() != null && role.getId() >= 0) {
            // 保存修改
            ServiceResult<Integer> result = roleService.update(new ServiceRequest(role));
            if (result.getErrorNO() != null) {
                dwzJson = new DwzJson("300", this.messageSource.getMessage(result.getErrorInfo(),
                    result.getMsgArgs(), this.getThisLocale()));
            } else {
                dwzJson = new DwzJson("200", this.messageSource.getMessage("operation.success",
                    result.getMsgArgs(), this.getThisLocale()), "dwz_tab_role",
                    EnumDwzJsonCallbackType.CLOSECURRENT.getCode());
            }
        } else {
            // 保存新增
            ServiceResult<Role> result = roleService.save(new ServiceRequest(role));
            if (result.getErrorNO() != null) {
                dwzJson = new DwzJson("300", this.messageSource.getMessage(result.getErrorInfo(),
                    result.getMsgArgs(), this.getThisLocale()));
            } else {
    ...
    @ResponseBody
    public DwzJson roleDel(@PathVariable("id") Long id, UserAgent user, ModelMap model) {
        DwzJson dwzJson;
        if (id == null || id < 0) {
            dwzJson = new DwzJson("300", this.getMessageSource().getMessage(
                "delete.error.parameter.ids.null", null, this.getThisLocale()));
        ...
        query.setId(id);
        ServiceResult<Integer> result = roleService.remove(new ServiceRequest(query));
        if (result.getErrorNO() != null) {
            dwzJson = new DwzJson("300", this.messageSource.getMessage(result.getErrorInfo(),
                result.getMsgArgs(), this.getThisLocale()));
        } else {
    ...
    public @ResponseBody
    DwzJson roleDelBatch(@ModelAttribute("query") RoleQuery query, UserAgent user, ModelMap model) {
        DwzJson dwzJson;
